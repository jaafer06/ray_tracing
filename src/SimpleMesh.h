...
public:
	...
	void normalize() {
		auto result = get_mesh_center_and_radius();
	...
	std::pair<Vector3f, float> get_mesh_center_and_radius() {
		Vector3f mesh_center = get_mesh_center();
		float mesh_radius = 0;
	...
	void resize() {
		float mesh_radius = get_mesh_center_and_radius().second;
		float factor = 2;
	...
	void transform(const Matrix3f& rotation, const Vector3f& translation) {
		for (Vertex& v : m_vertices) {
			v.position.head(3) = (rotation * v.position.head(3)) + translation;
		}
	}
	...
	bool loadMesh(const std::string& filename) {
		...
		if (std::string(string1).compare("COFF") == 0) {
			...
			for (unsigned int i = 0; i < numV; i++) {
				...
				colorInt /= 255;
				// v.color = Vector4f((unsigned char)colorInt.x(), (unsigned char)colorInt.y(), (unsigned char)colorInt.z(), (unsigned char)colorInt.w());
				v.color = colorInt;
				m_vertices.push_back(v);
			}
		}
	 ...
	 */
	static SimpleMesh sphere(Vector3f center, float scale = 1.f, Vector4f color = { 0, 0, 1, 1 }) {
		SimpleMesh mesh;
